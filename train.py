...
from utils.models import BertForSequenceTagging, BertForSeqClass
...
MODEL_CLASSES = {
    ...
    "bert-ex" : (BertConfig, BertForSeqClass, ExtendedBertTokenizer),
    # "bert-ex" : (BertConfig, BertForSequenceClassification, ExtendedBertTokenizer),
...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "pico": batch[3], "labels": batch[5]}
            # inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[5]}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if 'neoplasm' in args.data_dir: data_name = 'neoplasm'# args.data_dir.split('/')[-1]
        if 'glaucoma' in args.data_dir: data_name = 'glaucoma'
        if 'mixed' in args.data_dir: data_name = 'mixed'
        output_file = os.path.join(os.path.join(eval_output_dir, prefix), "sequence_tagging_predictions_"+data_name+".conll")
...
def main():
    ...
    args.do_eval = True
    args.do_train = False
    args.data_dir = 'data/neoplasm'
    print('EVALUATING NEOPLASM TEST SET ')
    if args.do_eval and args.local_rank in [-1, 0]:
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        checkpoints = [args.output_dir]
        if args.eval_all_checkpoints:
    ...
    args.do_eval = True
    args.do_train = False
    args.data_dir = 'data/glaucoma_test'
    print('EVALUATING GLAUCOMA TEST SET ')
    if args.do_eval and args.local_rank in [-1, 0]:
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        checkpoints = [args.output_dir]
        if args.eval_all_checkpoints:
    ...
    args.do_eval = True
    args.do_train = False
    args.data_dir = 'data/mixed_test'
    print('EVALUATING MIXED TEST SET ')
    if args.do_eval and args.local_rank in [-1, 0]:
        tokenizer = tokenizer_class.from_pretrained(args.output_dir, do_lower_case=args.do_lower_case)
        checkpoints = [args.output_dir]
        if args.eval_all_checkpoints:
