...
class ExtendedBertTokenizer(BertTokenizer):
    ...
    def tokenize_with_label_extension(self, text, pico, labels, copy_previous_label=False, extension_label='X'):
        '''
    ...
    def tokenize_with_pico(self, text, pico):
        text = text.split(' ')
        pico = pico.split(' ')
        new_text = []
        new_pico = []
...
class ExtendedPicoBertTokenizer(BertTokenizer):
    """BertTokenizer which extends a token-based list of labels to WordPiece sub-token-based list.
    Params, Inputs, Outputs:
        See base class.
    """
